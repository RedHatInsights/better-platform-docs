...
import fse, { ensureDirSync, move } from "fs-extra";
...
const adocExec = (file) => {
  return new Promise((res, rej) => {
    const outputHTML = file.replace(/\.adoc$/, ".html");
    // need three way transform via HTML -> XML -> MD to avoid invalid pandoc XML issues generated by asciidoctor
    exec(`asciidoctor -b html -r asciidoctor-diagram ${file}`, (error) => {
      if (error) {
        console.log(error);
        return rej(error);
      }
...
// prepare HTML parser template
const parserTemplate = readFileSync(
  "./components/consoledot-pages/pageTemplate.js",
  {
    encoding: "utf-8",
  }
);

const moveTemplate = (file) => {
  const target = file.split("/modules").pop().replace("/pages/", "/");
  ...
  return Promise.all([htmlPromise, writePromies]);
};
...
const transformerMapper = {
  "consoledot.pages.redhat.com": async ({
    ...
    const publicFolder = `./public/${title
      .replaceAll(" ", "-")
      .toLowerCase()}/${path.split("/").pop()}`;

    ensureDirSync(publicFolder);
    process.env.IMAGE_PREFIX = `/${title
      .replaceAll(" ", "-")
      .toLowerCase()}/${path.split("/").pop()}`;
    const promises = [];
    ...
    await promisesResult.forEach(moveTemplate);
    const images = [
