...
const adocExec = (file) => {
  return new Promise((res, rej) => {
    ...
    const outputDocbook = file.replace(/\.adoc$/, ".xml");
    const outputMd = file.replace(/\.adoc$/, ".md");
    // need three way transform via HTML -> XML -> MD to avoid invalid pandoc XML issues generated by asciidoctor
    exec(
      `asciidoctor -b html -r asciidoctor-diagram ${file} && pandoc -s --shift-heading-level-by=1 ${outputHTML} -o ${outputDocbook} -f html -t docbook --filter=./downloader/adoc-writer.js --verbose &&
      pandoc ${outputDocbook} -o ${outputMd} -f docbook -t gfm `,
      (error) => {
        if (error) {
          console.log(error);
          return rej(error);
