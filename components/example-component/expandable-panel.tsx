...
const Toolbar = dynamic(
  () => import("@patternfly/react-core").then((mod) => mod.Toolbar),
  { ssr: false }
);
...
const TimedToas: React.FC<{ handleClose: () => void; title: string }> = ({
  handleClose,
  title,
}) => {
...
const ExpandablePanel: React.FC<{
  ...
  source: string;
  sourceCode: string;
}> = ({ codeOnly, language, sourceCode, source }) => {
  const [isOpen, setIsOpen] = useState(!!codeOnly);
  const [toast, setToast] = useState<{ title: string } | undefined>(undefined);
              ...
              <div>
                ...
                >
                  <input
                    type="hidden"
                    name="parameters"
                    value={createCodeSandboxExample(sourceCode)}
                  />
                  <Button
